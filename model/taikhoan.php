...
function suainforuser($id,$email,$hoten,$phone,$avatar,$pass){
    if($avatar!="../image/avatar/"){
    $sql="UPDATE `taikhoan` SET 
    `email`='$email',`hoten`='$hoten',`phone`='$phone',`avatar`='$avatar',`pass`='$pass' WHERE id=".$id;
    update($sql);
    }else{
        $sql="UPDATE `taikhoan` SET 
    `email`='$email',`hoten`='$hoten',`phone`='$phone',`pass`='$pass' WHERE id=".$id;
      update($sql);
    }
}
