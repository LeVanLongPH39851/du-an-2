<div class="container-fluid">
    ...
    <form action="index.php?act=suasp" method="post" enctype="multipart/form-data">
        ...
        <div class="form-group">
            ...
            <input type="number" class="form-control" id="price" value="<?php echo $sp[0]['price']?>" name="price"
                required>
        </div>
        <div class="form-group">
            <label for="quantity">Số lượng</label>
            <input type="number" class="form-control" id="quantity" value="<?php echo $sp[0]['quantity']?>"
                name="quantity" required>
        </div>
        <div class="form-group">
            <label for="luotxem">Lượt xem</label>
            <input type="number" class="form-control" value="<?=$sp[0]['luotxem']?>" id="luotxem" name="luotxem"
                required>
        </div>
        <div class="form-group">
       ...
       if($sp[0]['id_dm']==$key['id']){
        $s = "selected";
      }else{
        $s="";
      }
        ?>
                <option value="<?php echo $key['id']?>" <?=$s?>><?php echo $key['name']?></option>
                <?php endforeach;?>
            </select>
        </div>
